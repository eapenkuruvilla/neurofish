...
OPTIMIZED VERSION - Pure C++ backend (no python-chess dependency)
...
class CachedBoard:
    ...
    def push(self, move_int: int) -> None:
        ...
        if is_null_move:
            ...
            is_ep = self._board.is_en_passant(cpp_move)
            is_castling = self._board.is_castling(cpp_move)
        ...
        if is_null_move:
            ...
            for m_int in self._move_stack:
                if m_int == 0:
                    ...
                    cpp_move = self._int_to_cpp_move(m_int)
                    self._board.push(cpp_move)
            return move_int
    ...
    def get_legal_moves_int(self) -> List[int]:
        ...
        if cache.legal_moves_int is None:
            ...
            for m in cpp_moves:
                move_int = self._cpp_move_to_int(m)
                result.append(move_int)
                gives_check_cache[move_int] = self._board.gives_check(m)
    ...
    def is_capture_int(self, move_int: int) -> bool:
        ...
        if move_int not in cache.move_is_capture_int:
            cpp_move = self._int_to_cpp_move(move_int)
            cache.move_is_capture_int[move_int] = self._board.is_capture(cpp_move)
    ...
    def is_en_passant_int(self, move_int: int) -> bool:
        ...
        if move_int not in cache.move_is_en_passant_int:
            cpp_move = self._int_to_cpp_move(move_int)
            cache.move_is_en_passant_int[move_int] = self._board.is_en_passant(cpp_move)
    ...
    def is_castling_int(self, move_int: int) -> bool:
        ...
        # Standard castling patterns
        if from_sq == E1 and to_sq in (G1, C1):
            piece = self._board.piece_at(E1)
            return piece is not None and piece.piece_type == KING
        if from_sq == E8 and to_sq in (G8, C8):
            piece = self._board.piece_at(E8)
            return piece is not None and piece.piece_type == KING
        return False
    ...
    def get_victim_type_int(self, move_int: int) -> Optional[int]:
        ...
        if move_int not in cache.move_victim_type_int:
            ...
            piece = self._board.piece_at(to_sq)
            if piece:
                cache.move_victim_type_int[move_int] = piece.piece_type
    ...
    def get_attacker_type_int(self, move_int: int) -> Optional[int]:
        """Get piece type of moving piece (for MVV-LVA)."""
        cache = self._cache_stack[-1]
        ...
        if move_int not in cache.move_attacker_type_int:
            from_sq = move_int & 0x3F
            piece = self._board.piece_at(from_sq)
            cache.move_attacker_type_int[move_int] = piece.piece_type if piece else None
    ...
    def precompute_move_info_int(self, moves_int: List[int]) -> None:
        """Pre-compute move info for a batch of moves."""
        cache = self._cache_stack[-1]
        if cache.move_is_capture_int is None:
            cache.move_is_capture_int = {}
        if cache.move_gives_check_int is None:
        ...
        for move_int in moves_int:
            if move_int in cache.move_is_capture_int:
                continue
            ...
            is_capture = board.is_capture(cpp_move)
            is_ep = board.is_en_passant(cpp_move)
            is_castling = self.is_castling_int(move_int)
            ...
            # Captured piece info
            if is_ep:
                cache.move_captured_piece_type_int[move_int] = PAWN
                cache.move_captured_piece_color_int[move_int] = not turn
                cache.move_victim_type_int[move_int] = PAWN
            elif is_capture:
                piece = board.piece_at(to_sq)
                if piece:
                    cache.move_captured_piece_type_int[move_int] = piece.piece_type
                    cache.move_captured_piece_color_int[move_int] = piece.color
                    cache.move_victim_type_int[move_int] = piece.piece_type
                else:
            ...
            # MVV-LVA score
            victim = cache.move_victim_type_int[move_int]
            attacker_type = cache.move_attacker_type_int[move_int]
            if victim and attacker_type:
                cache.move_mvv_lva_int[move_int] = _MVV_LVA[victim][attacker_type]
            else:
                cache.move_mvv_lva_int[move_int] = 0
